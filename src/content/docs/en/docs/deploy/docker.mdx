...
Confirm Docker, Compose, and Buildx are available; the project Dockerfiles use BuildKit cache mounts:
...
For MySQL, set `DATABASE_DRIVER=mysql` and use a `mysql://...` `DATABASE_URL`. The current gateway fixes the MySQL 8.4 ambiguous-column error in `0002_seed.sql`; if an old fork reports `Column 'key' in field list is ambiguous`, sync the latest migration before retrying.
...
1. Host ports: `docker/compose/node-*.yml` uses `GATEWAY_PROXY_HOST_PORT` / `GATEWAY_ADMIN_HOST_PORT`; `docker/examples/*.yml` uses `GATEWAY_PROXY_PORT` / `GATEWAY_ADMIN_PORT`. Containers still listen on `8787` / `8789`.
</Steps>
