...
例如，部分 OpenCode 兼容上游会校验 `x-opencode-session`。一些客户端无法补充这个 Header，或者传入的值不符合上游要求，都会导致请求被拒绝。通过 OctaFuse，只需在路由中配置正确的 Header 并开启请求头强制覆盖，接入这条路由的客户端无需逐个修改，也无法再覆盖这个固定值。
