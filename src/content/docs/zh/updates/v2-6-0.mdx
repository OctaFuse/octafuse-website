---
title: OctaFuse 2.6.0：完善 Vertex AI 鉴权、图片计费与日常调试
description: OctaFuse Gateway 2.6.0 完善项目级 Vertex AI 服务账号鉴权，明确文生图双模式计费与异常不扣费规则，并升级路由时段计费、Playground、Simulator 及供应商目录。
...
sidebar:
  hidden: true
site_external_id: "octafuse-release-v2-6-0"
site_post_id: 504
site_status: "published"
canonical_url: "https://didispace.com/posts/504"
---
...
![OctaFuse Provider 模板目录筛选出两种 Google Vertex AI 接入方式](/screenshots/v2-6-0-provider-import-vertex.webp)
...
![OctaFuse 模型页面按 Image 类型筛选，并展示按张与按 Token 两类价格](/screenshots/v2-6-0-model-image-billing.webp)

*模型列表会直接标出 `/img` 或 `/M`，便于在配置路由前先确认图片模型采用按张还是按 Token 计费。*
...
![OctaFuse 路由编辑器展示默认计费倍率与两个每日时段](/screenshots/v2-6-0-route-billing-schedule.webp)
...
![OctaFuse Playground 选择单条路由并载入 Tool stream 请求样例](/screenshots/v2-6-0-playground-tool-stream.webp)

*选择单条路由后可直接载入 Tool stream 等请求样例；Playground 不扣减用户预算、不写请求日志，也不执行路由池故障转移。*

![OctaFuse Simulator 根据 Gemini 模型请求入口提供 OpenAI 与 Gemini 协议选择](/screenshots/v2-6-0-simulator-routing-target.webp)

*Simulator 先选择客户端可见模型和路由组，再按匹配到的请求入口生成协议与请求路径；填写 Proxy 地址和测试密钥后即可验证完整链路。*

本次更新还扩充了供应商目录，新增多项国际站、官方模型 API、推理平台和聚合网关预设。官网的[供应商目录](/zh/catalog/providers/)与[模型目录](/zh/catalog/models/)已经同步到 v2.6.0。
...
升级前可查看 [GitHub Release v2.6.0](https://github.com/OctaFuse/octafuse-gateway/releases/tag/v2.6.0) 与[完整 Changelog](https://github.com/OctaFuse/octafuse-gateway/blob/v2.6.0/CHANGELOG.md)。
